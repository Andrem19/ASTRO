...
def test_profile_day_forecast_registered_as_mcp_tool() -> None:
    ...
    assert "calculate_profile_natal_chart" in tool_names
...
def test_astro1_aliases_are_not_registered_for_mcp_tools() -> None:
    tools = asyncio.run(create_mcp_server(Settings()).list_tools())
    tool_names = {tool.name for tool in tools}

    assert all(not name.startswith("astro1_") for name in tool_names)
